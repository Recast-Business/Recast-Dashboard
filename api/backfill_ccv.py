...
Body (optional): {"limit": 50}  — max creators to process per call (default 50).
"""
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            body = read_body(self)
            batch_limit = min(int(body.get("limit") or 50), 100)
            ...
            # Find creators needing CCV backfill
            twitch_needs = []  # (row_num, login)
            kick_needs = []    # (row_num, handle)

            for i, row in enumerate(all_values[1:], start=2):
                if len(twitch_needs) + len(kick_needs) >= batch_limit:
                    break
                # Twitch: has handle, no CCV
